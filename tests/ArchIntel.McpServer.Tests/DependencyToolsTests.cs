...
public sealed class DependencyToolsTests
{
    ...
    public async Task FindDependencies_ReturnsDirectDependencies_WithGraphVersionStamp()
    {
        ...
        Assert.Contains(result.Dependencies, d => d.Relationship == "Implements" && d.Node.Name == "IOrderService");
        Assert.Contains(result.Dependencies, d => d.Relationship == "Injects" && d.Node.Name == "IOrderRepository");
